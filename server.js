...
// Apollo's error bodies carry a machine-readable code — surface the
...
function isGenericLocal(local) {
  ...
  // "2308@", "2024@" — pure-numeric locals are device/session/test IDs, not people.
...
// Well-known people-ish pages tried in order before internal-link discovery.
const CRAWL_PATHS = ['/about', '/contact', '/team', '/about-us', '/contact-us',
  '/our-team', '/team-members', '/leadership', '/people', '/staff', '/company',
  '/who-we-are', '/meet-the-team', '/impressum', '/aboutus'];
// Internal links on the homepage worth following when guessing pages.
...
function discoverLinks(html, domain) {
  ...
  while ((m = re.exec(html))) {
    ...
    if (/^https?:\/\//i.test(link)) {
      try { link = new URL(link).pathname; } catch { continue; }
      const host = (m[1].match(/^https?:\/\/([^/]+)/i) || [])[1] || '';
      const bare = host.replace(/^www\./, '').toLowerCase();
      if (bare !== domain.replace(/^www\./, '')) continue;
    }
    if (!link.startsWith('/')) continue;
    if (PERSONISH_LINK.test(link) && !out.includes(link)) out.push(link);
  }
  return out;
}
...
async function builtinSearchLeads(f, perPage, sessionEmail) {
  ...
  for (const domain of domains) {
    ...
    for (const path of queue) {
      if (found.size >= 10 || pages >= MAX_PAGES_PER_DOMAIN) break;
      pages++;
      try {
        const html = await crawlPage(domain, path);
...
function extractMarketPhrases(highText, domainName = '') {
  ...
  const push = (p, kind) => {
    ...
    // self-description, not a market ("acme is the leading…").
...
// One-time services (e.g. DNS setup done-for-you) ride the same
// checkout/activate flow; `service: true` marks non-credit packs.
const TOPUP_PACKS = {
...
function creditTopup(email, packId) {
  ...
  if (!pack) return null;
  if (pack.service) return fulfillService(email, packId);
  const users = load('users');
  const user = users.find(u => u.email === email.trim().toLowerCase());
  if (!user) return null;
...
// One-time service fulfillment (DNS setup pack): recorded purchases are
// listed in Settings alongside the billing actions the admin runs.
function fulfillService(email, packId) {
  const pack = TOPUP_PACKS[packId];
  if (!pack?.service) return null;
  const users = load('users');
  const user = users.find(u => u.email === email.trim().toLowerCase());
  if (!user) return null;
  user.servicePurchases = [...(user.servicePurchases || []), { pack: packId, name: pack.name, price: pack.price, at: new Date().toISOString() }];
  save('users', users);
  return user;
}
